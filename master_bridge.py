...
class MasterBridge(Node):
    def __init__(self):
        ...
        self.get_logger().info('MASTER-BRÜCKE V_FINAL: Echte Zeit, Odom ungedrosselt, RAM geschützt!')
        self.last_pc_time = 0.0

    def odom_cb(self, msg):
        # KEINE DROSSEL! Odometrie rast mit voller Geschwindigkeit!
        t = TransformStamped()
        
        # ECHTE LAPTOP-ZEIT FÜR DEN TF-BAUM
    ...
    def pc_cb(self, msg):
        ...
        # 10 Hz Drossel: Das rettet deinen RAM!
        if current_time - self.last_pc_time < 0.1:
            return
            
        self.last_pc_time = current_time

        scan = LaserScan()
        # ECHTE LAPTOP-ZEIT FÜR LIDAR
